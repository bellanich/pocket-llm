"""
Implementation of LLaVaOnevision Model
Implements the Siglip Vision Encoder. Uses QWen2 for the Language Encoder.
"""
...
class LlavaOnevisionConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    """
    LLaVa Config
    """
    ...
    text_architecture: str = "QWen2LMHeadModel"
...
class LlavaOnevisionForCausalLM(Module):
    def __init__(self, config: LlavaOnevisionConfig):
        super().__init__()
        self.config = config
        self.config.text_config.vocab_size = config.vocab_size
        self.vision_tower = SiglipVisionModel(config.vision_config)
        self.image_processor = ImageProcessor()
        self.multi_modal_projector = LlavaOnevisionMultiModalProjector(config)
        self.image_newline = nn.Parameter((config.text_config.hidden_size,))
    ...
    def image_embed(self, pixel_values: Tensor) -> Tensor:
        ...
        image_newline = broadcast_to(
            self.image_newline, shape=(1, self.config.text_config.hidden_size)
        )  # shape of (1,embed_dim)
        image_features = concat([image_features, image_newline], dim=0)
